import{r as s}from"./vendor-react-DhTDDHjY.js";function B(d,o,u){const S=s.useMemo(()=>{const n={critical:[],urgent:[],warning:[],priorityCritical:[],overstock:[]};return d.forEach(e=>{if((e.part_number||e["part number"]||"").toLowerCase()==="region")return;const p=(e["20_top_usage"]||e["20 top usage"]||"").toLowerCase()==="yes";Object.keys(e).filter(t=>t.toLowerCase().includes("idfsl")||t.toLowerCase().includes("idccw")).forEach(t=>{const a=parseInt(e[t]||0);let l=t;if(t.includes("idfsl")&&t.includes("_fsl_")){const A=t.split("_fsl_")[1];A&&(l="FSL "+A.split("_").map(y=>y.charAt(0).toUpperCase()+y.slice(1)).join(" "))}else if(t.includes("idccw")){const A=t.replace(/^idccw\d+_/,"");A&&(l=A.split("_").map(y=>y.charAt(0).toUpperCase()+y.slice(1)).join(" "),t.toLowerCase().includes("idccw00")&&(l+=" (Jakarta)"))}const i={part:e,fslName:l,fslColumn:t,stock:a};a===0?n.critical.push(i):a>=1&&a<=5?(n.urgent.push(i),p&&n.priorityCritical.push(i)):a>=6&&a<=10?(n.warning.push(i),p&&n.priorityCritical.push(i)):a>100&&n.overstock.push(i)})}),{critical:n.critical,urgent:n.urgent,warning:n.warning,priorityCritical:n.priorityCritical,overstock:n.overstock,criticalCount:n.critical.length,urgentCount:n.urgent.length,warningCount:n.warning.length,priorityCriticalCount:n.priorityCritical.length,overstockCount:n.overstock.length,totalLowStock:n.critical.length+n.urgent.length+n.warning.length}},[d]),g=s.useMemo(()=>o.reduce((n,e)=>n+parseInt(e.grand_total||e["grand total"]||0),0),[o]),P=s.useMemo(()=>{const n={};return u.forEach(e=>{const r=e.region||e["region "]||"Unknown";n[r]=(n[r]||0)+1}),Object.entries(n).map(([e,r])=>({name:e,value:r}))},[u]),U=s.useMemo(()=>{const n={};return o.forEach(e=>{if((e.part_number||e["part number"]||"").toLowerCase()==="region")return;const p=e.type_of_part||e["type of part"]||"Other",c=parseInt(e.grand_total||e["grand total"]||0);n[p]=(n[p]||0)+c}),Object.entries(n).map(([e,r])=>({name:e,value:r})).sort((e,r)=>r.value-e.value).slice(0,5)},[o]),k=s.useMemo(()=>o.length?o.filter(n=>{const e=n.part_number||n["part number"]||"";return e&&e.toLowerCase()!=="region"}).map(n=>{const e=parseInt(n.grand_total||n["grand total"]||0);return{name:(n.part_name||n["part name"]||"Unknown").substring(0,20),partNumber:n.part_number||n["part number"],value:e}}).sort((n,e)=>e.value-n.value).slice(0,8):[],[o]),R=s.useMemo(()=>o.filter(n=>{var e;return((e=n["20_top_usage"])==null?void 0:e.toLowerCase())==="yes"}).length,[o]);return{stockAlerts:S,totalStockQuantity:g,regionDistribution:P,stockByType:U,topPartsByStock:k,top20UsageParts:R,totalFSL:u.length,totalParts:o.length}}const D=`{
